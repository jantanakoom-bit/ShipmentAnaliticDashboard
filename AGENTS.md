...
Use `npm install` once to install dependencies. Run `npm run dev` for the Vite frontend and `npm run dev:server` in a second terminal for the Express API on port `3001`; the frontend dev server proxies `/api` to the backend. Create a production bundle with `npm run build`, then validate it locally with `npm run preview`. Run unit and API tests with `npm test`, or target layers with `npm run test:unit` and `npm run test:api`. Run browser coverage with `npm run test:e2e`. Generate password hashes with `npm run hash-password -- <plain-text-password>`. For container work, use `docker compose up --build frontend` or `docker compose --profile backend up --build`.
